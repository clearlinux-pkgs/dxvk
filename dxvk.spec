#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : dxvk
Version  : 1.5.4
Release  : 6
URL      : https://github.com/doitsujin/dxvk/archive/v1.5.4/dxvk-1.5.4.tar.gz
Source0  : https://github.com/doitsujin/dxvk/archive/v1.5.4/dxvk-1.5.4.tar.gz
Summary  : No detailed summary available
Group    : Development/Tools
License  : BSD-3-Clause Zlib
Requires: dxvk-lib = %{version}-%{release}
Requires: dxvk-license = %{version}-%{release}
BuildRequires : Vulkan-Loader-dev
BuildRequires : Vulkan-Loader-dev32
BuildRequires : buildreq-meson
BuildRequires : gcc-dev32
BuildRequires : gcc-libgcc32
BuildRequires : gcc-libstdc++32
BuildRequires : glibc-dev32
BuildRequires : glibc-libc32
BuildRequires : glslang
BuildRequires : mingw-binutils
BuildRequires : mingw-crt
BuildRequires : mingw-crt-dev
BuildRequires : mingw-gcc
BuildRequires : pkgconfig(32vulkan)
BuildRequires : pkgconfig(vulkan)
BuildRequires : wine
BuildRequires : wine-dev
BuildRequires : wine-dev32
BuildRequires : wine-extras-staticdev
Patch1: 74d23c22dea0445d33345de36a1cc7ee531f94d5.patch

%description
# DXVK
A Vulkan-based translation layer for Direct3D 9/10/11 which allows running 3D applications on Linux using Wine.

%package lib
Summary: lib components for the dxvk package.
Group: Libraries
Requires: dxvk-license = %{version}-%{release}

%description lib
lib components for the dxvk package.


%package lib32
Summary: lib32 components for the dxvk package.
Group: Default
Requires: dxvk-license = %{version}-%{release}

%description lib32
lib32 components for the dxvk package.


%package license
Summary: license components for the dxvk package.
Group: Default

%description license
license components for the dxvk package.


%prep
%setup -q -n dxvk-1.5.4
cd %{_builddir}/dxvk-1.5.4
%patch1 -p1

%build
## build_prepend content
meson --cross-file build-wine64.txt --buildtype release --prefix `pwd -P`/64 build.w64
pushd build.w64
ninja
popd

meson --cross-file build-wine32.txt --buildtype release --prefix `pwd -P`/32 build.w32
pushd build.w32
ninja
popd
## build_prepend end
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1582652075
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FCFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=4 "
make  %{?_smp_mflags}  ||:


%install
export SOURCE_DATE_EPOCH=1582652075
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/package-licenses/dxvk
cp %{_builddir}/dxvk-1.5.4/LICENSE %{buildroot}/usr/share/package-licenses/dxvk/7ad85429f3b5fa8bbe1d97211f3c3e780038a1a9
cp %{_builddir}/dxvk-1.5.4/include/openvr/LICENSE %{buildroot}/usr/share/package-licenses/dxvk/99e40ce3676d57f631fca8692017203185ab6b5f
%make_install ||:
## install_append content
mkdir -p %{buildroot}/usr/lib64/wine
find build.w64/src -name '*.dll.so' -exec install {} %{buildroot}/usr/lib64/wine/ \;
mkdir -p %{buildroot}/usr/lib32/wine
find build.w32/src -name '*.dll.so' -exec install {} %{buildroot}/usr/lib32/wine/ \;
## install_append end

%files
%defattr(-,root,root,-)

%files lib
%defattr(-,root,root,-)
/usr/lib64/wine/d3d10.dll.so
/usr/lib64/wine/d3d10_1.dll.so
/usr/lib64/wine/d3d10core.dll.so
/usr/lib64/wine/d3d11.dll.so
/usr/lib64/wine/d3d9.dll.so
/usr/lib64/wine/dxgi.dll.so

%files lib32
%defattr(-,root,root,-)
/usr/lib32/wine/d3d10.dll.so
/usr/lib32/wine/d3d10_1.dll.so
/usr/lib32/wine/d3d10core.dll.so
/usr/lib32/wine/d3d11.dll.so
/usr/lib32/wine/d3d9.dll.so
/usr/lib32/wine/dxgi.dll.so

%files license
%defattr(0644,root,root,0755)
/usr/share/package-licenses/dxvk/7ad85429f3b5fa8bbe1d97211f3c3e780038a1a9
/usr/share/package-licenses/dxvk/99e40ce3676d57f631fca8692017203185ab6b5f
